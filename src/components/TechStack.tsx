		...
		<div className=" w-full p-8 rounded-3xl h-auto mt-10 mb-20 px-1 md:px-20">
			...
			<div className="flex flex-wrap  justify-center gap-6 mt-20">
				{techStacks.map(({ icon, techname }: { icon: string; techname: string }, index) => (
					<BackgroundGradient className="bg-black rounded-xl " containerClassName="w-fit rounded-xl " key={index}>
						<div className=" w-[100px] h-[100px] md:w-[120px] md:h-[120px] flex flex-col justify-center items-center px-2 py-1 rounded-xl ">
							...
							<p className="text-sm md:text-base uppercase text-center">{techname}</p>
						</div>
					</BackgroundGradient>
				))}
			</div>
